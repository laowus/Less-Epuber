...
// 检查 curChapter 是否成功注入
if (!curChapter) {
  console.error("curChapter 注入失败");
}
...
const parseEpub = async (file) => {
  ...
  console.log("传递给 open 函数的 file 对象:", file);
  let res;
  try {
    res = await open(file);
    console.log("open 函数正常返回:", res);
  } catch (error) {
    console.error("open 函数执行出错:", error);
    ElMessage.error("解析 EPUB 文件失败，请检查文件是否损坏");
    return;
  }
  if (res) {
    console.log("res", res);
...
const initDom = () => {
  ...
  $("#add-epub-btn").addEventListener("click", () =>
    $("#add-epub-file").click()
  );
};
onMounted(() => {
...
const updateChapters = () => {
  ...
  const foundIndex = chapters.value.findIndex(
    (chap) => chap?.index === targetIndex
  );
  if (foundIndex !== -1) {
    // 找到对应的 index，更新该对象
...
const saveMetadata = () => {
  ...
  if (metadata.value.ext === ".txt") {
    curChapter.value = {
      ...
      content: metadata.value.fileContent,
    };
    chapters.value = [];
    chapters.value.push(curChapter.value);
    console.log(chapters.value);
